...
var missileMaterialCost : int;
var missileMaterialDescrip = "Faster missile production";
...
function OnMouseEnter(){
	...
	GameController.techCost = missileMaterialCost;
	GameController.descripText = missileMaterialDescrip;
}
...
function OnMouseDown(){
	if ((DestroyByContact.money >= missileMaterialCost) && (!button2Selected)){
		buttonPick += 1;
		GetComponent.<Renderer>().material.mainTexture = button2Down;
 		GameController.missileBuildSpeed -= 3;
 	 	GameController.techsResearched += 1;
 	 	DestroyByContact.money = DestroyByContact.money - missileMaterialCost;
 	 	missileMaterialCost += missileMaterialCost;
 	 	if (buttonPick > 3){
